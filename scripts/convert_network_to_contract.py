#!/usr/bin/env python3
"""Convert legacy network telemetry JSONL files into Temporal Contract v1 JSONL (network stream).

Usage:
  python scripts/convert_network_to_contract.py input.jsonl [--out output.jsonl] [--session-id SESSION]
  or
  python scripts/convert_network_to_contract.py --dir CompuCogLogger/logs/network

Behavior:
- Emits session_start (seq=1)
- Wraps each line as a contract 'network' event
- Emits session_end
- t_monotonic_ns captured at read time; t_utc is current UTC
- payload contains original object under 'network'
...
def convert_file(input_path: Path, output_path: Path, session_id: str = None, source: str = 'network') -> int:
    ...
    with output_path.open('w', encoding='utf-8') as out_f, input_path.open('r', encoding='utf-8') as in_f:
        ...
        for line in in_f:
            ...
            payload['network'].setdefault('_converted_from', input_path.name)
            evt = make_event(source, session_id, seq, payload, event_type='network')
            out_f.write(json.dumps(evt, ensure_ascii=False) + "\n")
            seq += 1
...
def main():
    p = argparse.ArgumentParser(description='Convert legacy network JSONL to Temporal Contract v1 JSONL')
    p.add_argument('input', type=str, nargs='?', default=None, help='input legacy JSONL file or directory (defaults to CompuCogLogger/logs/network)')
    ...
    if args.input:
        ...
        dirp = Path('CompuCogLogger/logs/network')
