...
# Header layout from spec (SCHEMA v2 - native resolution support):
# 0   4  MAGIC (uint32)
# 4   2  VERSION (uint16)
...
# 38  2  GRID_SHAPE_IN_H (uint16) -- UPGRADED from uint8 for native resolution
# 40  2  GRID_SHAPE_IN_W (uint16) -- UPGRADED from uint8 for native resolution
# 42  2  GRID_SHAPE_OUT_H (uint16) -- UPGRADED from uint8 for native resolution
# 44  2  GRID_SHAPE_OUT_W (uint16) -- UPGRADED from uint8 for native resolution
# 46  1  COLOR_COUNT (uint8)
# 47  1  TRAIN_PAIR_COUNT (uint8)
#
# Total fixed header size: 48 bytes (was 44 in v1)
_HEADER_FMT = "<IHHQBI dB HHHH HHHH BB"
...
class ForgeRecord:
    ...
    def serialize(self, string_dict: StringDictionary) -> bytes:
        ...
        # v2 schema: grid dimensions are uint16 for native resolution support
        header = struct.pack(
            _HEADER_FMT,
            MAGIC_RECORD,
            ...
            int(grid_h_in) & 0xFFFF,   # uint16 for native resolution
            int(grid_w_in) & 0xFFFF,   # uint16 for native resolution
            int(grid_h_out) & 0xFFFF,  # uint16 for native resolution
            int(grid_w_out) & 0xFFFF,  # uint16 for native resolution
            int(self.color_count) & 0xFF,
            train_pair_count & 0xFF,
        )
        assert len(header) == _HEADER_SIZE == 48  # v2 header is 48 bytes
