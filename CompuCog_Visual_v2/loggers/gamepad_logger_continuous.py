...
try:
    from logger_common import startup_cleanup
except ImportError:
...
class ContinuousGamepadLogger:
    ...
    def poll_and_log(self):
        ...
        for controller_id, controller in enumerate(self.controllers):
            try:
                ...
                # Controller disconnected or hardware error - log once
                if not hasattr(self, '_pygame_error_logged'):
                    print(f"\n[Gamepad] pygame error: {e}")
    ...
    def _reinit_controllers(self):
        """Reinitialize controllers after disconnect/error."""
        try:
    ...
    def run(self):
        """
        IMMORTAL continuous logging loop - runs FOREVER.
        ...
        print(f"[Gamepad] IMMORTAL mode - will never exit")
        
        # Auto-cleanup old data (older than 1 day)
        if startup_cleanup:
            try:
                logs_base = Path(self.log_path).parent.parent
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # Controller disconnected - try reinit every 5 seconds
                    if time.time() - last_reinit_time > 5.0:
                        self._reinit_controllers()
                        last_reinit_time = time.time()
                except Exception:
                    pass
                ...
                print("\n[Gamepad] Ctrl+C ignored - IMMORTAL mode")
                time.sleep(0.1)
                continue
            
            except Exception as e:
                # ANY error - log and continue
                print(f"\n[Gamepad] Error (continuing): {e}", file=sys.stderr)
                time.sleep(0.1)
                continue
