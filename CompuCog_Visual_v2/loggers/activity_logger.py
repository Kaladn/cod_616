...
def collect_and_log(output_path: str = None):
    ...
    logging.info(f"CompuCogLogger - User Activity [IMMORTAL]")
    ...
    while True:
        try:
            ...
            # Check if date rolled over (only if using default path)
            if not output_path:
                try:
                    ...
                    pass  # Keep using current file
            
            # Get idle duration (safe)
            try:
                idle_seconds = get_idle_duration()
            except Exception:
                idle_seconds = 0.0
            
            # Get active window info (safe)
            try:
                hwnd, pid, window_title = get_active_window()
            except Exception:
                hwnd, pid, window_title = None, None, ""
            
            # Get process info (safe)
            try:
                if pid:
            ...
            # Write (immortal - never crashes)
            write_safe(log_file, log_entry)
            
            # Status update every 20 samples (~1 minute)
            if sample_count % 20 == 0:
                status = "IDLE" if idle_seconds > 5.0 else "ACTIVE"
                logging.info(f"[{datetime.now().strftime('%H:%M:%S')}] {sample_count} events | {status} | {process_name or 'Unknown':<30}")
